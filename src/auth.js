...
if (location.href.indexOf('localhost:3000') > -1) {
  ...
  URL = 'https://taskpenguin.surge.sh';
} 
// exchange the object with your own from the setup step above.
const webAuth = new auth0.WebAuth({
  domain: 'taskpenguin.au.auth0.com',
  clientID: 'iIFgc2BpMxECqzeV45IepoxS5PW1S774',
  // make sure this line is contains the port: 8080
  redirectUri: `${URL}/callback`,
  // we will use the api/v2/ to access the user information as payload
  audience: 'https://taskpenguin.au.auth0.com/api/v2/',
  ...
  methods: {
    ...
    logout() {
      return new Promise(async (resolve, reject) => {
        ...
        webAuth.logout({
          returnTo: URL,
          client_id: 'iIFgc2BpMxECqzeV45IepoxS5PW1S774',
        });
      });
    },
    isAuthenticated() {
    ...
    handleAuthentication(err, authResult) {
      if (authResult && authResult.accessToken && authResult.idToken) {
        ...
        webAuth.client.userInfo(
          ...
          (err, user) => {
            this.user = user;
            //window.location.href = location.href.replace(/callback.*/, '');
            window.location.href = location.href.replace(/callback.*/, '');
          },
        );
      } else if (err) {
